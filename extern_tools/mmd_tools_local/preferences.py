...
def _get_update_candidate_branches(_, __):
    updater = mmd_tools_local.operators.addon_updater.AddonUpdaterManager.get_instance()
...
class MMDToolsAddonPreferences(bpy.types.AddonPreferences):
    ...
    def draw(self, _context):
        ...
        if not updater.candidate_checked():
            col = update_col.column()
            col.scale_y = 2
            col.operator(mmd_tools_local.operators.addon_updater.CheckAddonUpdate.bl_idname, text="Check mmd_tools_local add-on update", icon="FILE_REFRESH")
        else:
            ...
            if updater.update_ready():
                col.enabled = True
                col.operator(
                    mmd_tools_local.operators.addon_updater.UpdateAddon.bl_idname, text=bpy.app.translations.pgettext_iface("Update to the latest release version ({})").format(updater.latest_version()), icon="TRIA_DOWN_BAR"
                ).branch_name = updater.latest_version()
            else:
                col.enabled = False
                col.operator(mmd_tools_local.operators.addon_updater.UpdateAddon.bl_idname, text="No updates are available")

            update_col.separator()
            update_col.label(text="(Danger) Manual Update:")
            row = update_col.row(align=True)
            row.prop(self, "updater_branch_to_update", text="Target")
            row.operator(mmd_tools_local.operators.addon_updater.UpdateAddon.bl_idname, text="Update", icon="TRIA_DOWN_BAR").branch_name = self.updater_branch_to_update
